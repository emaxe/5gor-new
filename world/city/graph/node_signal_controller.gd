...
## (`(i, axis)` против `(node, approach)`) несовместимы, и живой сеточный
## город до этапа 9 продолжает пользоваться осевой без единой правки.
##
...
## что `TrafficLightController`. Это не совпадение и не украшение: до этапа 9
## линзы светофоров красит осевой контроллер (`city_builder.gd:252`), а
## машины с этого этапа слушают узловой, и разойдись они — игрок увидел бы
## машины, стоящие на зелёный. Паритет закреплён тестом
## `test_grid_node_matches_axis_controller`.
##
...
## и сеточный город не меняет поведения. Одиночный западный узел дал бы
## манхэттенское расстояние с лишним слагаемым по z — волна пошла бы по
