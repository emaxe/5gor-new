...
## (`front_nodes`): «откуда идёт волна» — такие же данные города, как список
## регулируемых узлов, и выводить их из габарита графа неправильно.
##
## Оговорка, чтобы не приписывать этой правке чужой заслуги: на нынешней
## топологии она НЕ меняет ни одного числа. В полосу `FRONT_BAND` там попадает
## ровно один узел, и это оказывается тот же самый западный въезд по проспекту
## Кирова, а расстояние здесь считается Дейкстрой по длинам рёбер, а не
## манхэттеном, — так что волна и по полосе шла вдоль проспекта правильно.
## Правка нужна не для сегодняшнего результата, а для устойчивости: стоит
## добавить в топологию узел западнее нынешнего минимума (продлить
## Октябрьскую, Дунаевского), и полоса молча перенесёт фронт на чужую улицу,
## не сломав ни одного теста. Явный список так сдвинуть нельзя.
##
## Полоса остаётся запасным путём для графов без списка (сетка 9x9 тестов,
## синтетические графы).
##
