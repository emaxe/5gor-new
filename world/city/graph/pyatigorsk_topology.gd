...
const W_LANE := 8.0
## Серпантин: 2 * CityField.SERP_ROAD_HALF (city_field.gd:36) — ровно та
## полоса, в которой оригинал считает серпантин проезжей частью.
const W_SERPENTINE := 8.8

## Клиренс путепровода у вокзала над улицей Крайнего, м. Должен быть строго
## больше 2 * CityGraph.LEVEL_TOLERANCE (2 * 2 м), иначе точка ровно между
## ярусами подойдёт обоим и запрос перестанет их разводить.
const OVERPASS_CLEARANCE := 6.5
...
func _vokzal_streets() -> void:
	...
	_chain("", [&"kir_e1", &"krn_m"], W_STREET)
	_chain("", [&"kir_e1", &"vok_e", &"krn_e"], W_STREET)
	_chain("", [&"pir_e", &"vok_e"], W_LANE)
	_chain("", [&"kra_past", &"kir_e1"], W_LANE)
	# Пандусы остаются на ярусе 0 (насыпь), пролёт — ярус 1. Уровень ребра
	# один на всё ребро, поэтому подъём делится именно так: у пандуса нижний
	# конец на земле, у пролёта оба конца на эстакаде.
	_chain("улица Козлова", [&"kal_s1", &"koz_d1"], W_STREET)
	_link("путепровод на улице Козлова", &"koz_d1", &"koz_d2", W_STREET,
		CityGraph.EdgeKind.BRIDGE, PackedVector2Array(), 1)
	_chain("улица Козлова", [&"koz_d2", &"koz_b"], W_STREET)
...
func _deck_node(id: StringName, x: float, z: float, district: StringName) -> void:
	_add(id, Vector3(x, OVERPASS_CLEARANCE, z), district,
...
func _polyline(a: int, b: int, via: PackedVector2Array) -> PackedVector3Array:
	...
	var dense := _densify(plan, TERRAIN_STEP) if terrain else plan
