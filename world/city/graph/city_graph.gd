...
## его. Но [b]запросы не реентерабельны[/b]: `nearest_edge` кладёт результат
## в поля `hit_*` и переиспользует буфер `SpatialHash2D`, чтобы не
## аллоцировать в hot path (трафик и пешеходы дёргают запросы каждый кадр на
## десятки агентов). Один поток — один граф; если позже понадобится опрос из
## нескольких потоков, заводить по курсору на поток, а не мьютекс.
...
var _hit_dict: Dictionary = {"edge_id": -1, "t": 0.0, "side": 0, "dist": INF}
...
## отрицательные — тоннели). Он отличает «два узла с близкой (x, z) на
## разных уровнях» по данным, не сравнивая саму высоту: у съезда на эстакаду
## высоты соседних ярусов сходятся, а level — нет.
...
## Ближайшее ребро: `{edge_id, t, side, dist}`. Словарь переиспользуется —
## копировать, если результат нужен между кадрами. В горячих циклах звать
## `query_nearest_edge()` и читать поля `hit_*`.
func nearest_edge(pos: Vector3, max_dist: float = 0.0) -> Dictionary:
	query_nearest_edge(pos, max_dist)
	_hit_dict["edge_id"] = hit_edge
	_hit_dict["t"] = hit_t
	_hit_dict["side"] = hit_side
	_hit_dict["dist"] = hit_dist
	return _hit_dict
...
## Приоритет при почти совпадающих расстояниях (кольцо и короткий рукав
## съезда касаются друг друга): сперва расстояние, при разнице меньше
## TIE_EPSILON — более широкое ребро, при равной ширине — меньший id.
##
## Широкое побеждает потому, что съезд лишь задевает кольцо: агент,
## физически едущий по кольцу, должен остаться на кольце. Меньший id —
## только чтобы выбор был детерминированным, а не зависел от порядка обхода
## ячейки хеша.
