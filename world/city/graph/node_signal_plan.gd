...
##
## [b]Пока standalone.[/b] Живой сеточный конвейер (`CityPlanner`/
## `CityBuilder`) до этапа 9 продолжает ставить осевые стойки: замена
## `_plan_signals()`/`_build_signals()` на этот план — работа этапа 9
## («зависимые системы»), вместе с GPS, миникартой и полицией.
