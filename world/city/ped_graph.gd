...
## Запас хорды перехода кольца над внешней кромкой аннулюса, м. Тот же смысл,
## что `DEAD_END_CLEARANCE` у обвода: неравенство держится строго, а сантиметр
## запаса не даёт ему лечь на кромку впритык, где решает погрешность sqrt/asin.
const RING_CHORD_MARGIN := 0.01
...
## рукавам своего сектора. (Основной член радиуса тротуара вокруг кольца
## считается иначе, см. `_ring_radius`: там полотно кольцевое, а не рукав; но
## та же величина входит туда вторым слагаемым — проверкой хорды перехода.)
...
##
## Хорда такого перехода (между `right = false` и `right = true` одного k)
## отстоит от центра кольца на `sqrt(ring² - side_k²)`. Это ДОЛЖНО быть больше
## внешней кромки аннулюса `radius + ring_half`, иначе пешеход переходил бы
## кольцо, а не рукав — `_ring_radius` гарантирует это явной проверкой хорды
## самого широкого рукава узла, а не только запасом тротуара.
...
## кольцевое. Дальше поднят до минимума, при котором хорда перехода САМОГО
## широкого рукава узла (см. `_ring_kerb_pos`) не заходит в аннулюс.
##
...
##
## [b]Второе слагаемое `maxf` — не косметика.[/b] `radius + ring_half +
## walk / 2` держит равномерный запас тротуара по всей окружности, но НЕ
## гарантирует хорду перехода: на живой топологии (`kir_vokzal`, `kal_s2`)
## честный радиус давал хорду перехода на 3-9 см ВНУТРИ внешней кромки
## аннулюса — тесты этого не ловили, потому что гоняют синтетику с
## одинаковыми ширинами рукавов. `sqrt(outer² + side_max²)` — минимальный
## радиус, при котором хорда самого широкого рукава проходит строго снаружи
## `outer` (см. вывод в `_ring_kerb_pos`).
func _ring_radius(node: int) -> float:
	var outer := _graph.node_radius(node) + _graph.ring_half(node)
	var side_max := _side_of_node(node)
	var chord_min := sqrt(outer * outer + side_max * side_max) + RING_CHORD_MARGIN
	return maxf(outer + walk_width * 0.5, chord_min)
...
##
## Итерация сходится геометрически (множитель `cos(излома)` от недостачи за
## шаг) для станции на изломе, где направление — биссектриса. Для серединной
## станции направление — нормаль СВОЕГО сегмента, и если ближайшим окажется
## сосед за изломом острее прямого угла, множитель становится `cos(2 * излома)`
## и при изломе больше 90° расходится — это дефект полилинии (излом развёрнут
## назад), а не тротуара.
##
## `push_error` вместо голого `assert`: `assert` вырезается в release, и
## расходимость молча уводила бы станцию на десятки метров — либо в полотно,
## либо посреди квартала, без единой диагностики. При расходимости откат к
## неитерированной точке `p + dir * side`: она не дальше корректного выноса и
## не убегает следом за расходящейся итерацией.
func _push_out(e: int, p: Vector3, dir: Vector3) -> Vector3:
	...
	while gap > MID_PUSH_EPS and steps < MID_PUSH_STEPS:
		...
		steps += 1
	if gap > MID_PUSH_EPS:
		push_error("PedGraph: ребро %d — серединный узел ленты не удалось вынести из полотна за %d шагов, недостача %.3f м" % [e, MID_PUSH_STEPS, gap])
		assert(false, "PedGraph: _push_out разошёлся, см. push_error выше")
		return p + dir * side
	return out
