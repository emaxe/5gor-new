...
##
## [b]Живой светофор города — не этот, а `NodeSignalController` (этап 9).[/b]
## Осевая адресация (индекс оси, ось дороги) на настоящей топологии не
## определена: у узла степени 3 или 5 осей нет. Класс остался единственным
## потребителем в лице `PoliceManager` (проверка проезда на красный) и умрёт
## вместе с её переводом на узловую адресацию.
##
...
## которую пешеход пересекает. Живые пешеходы этим методом больше не
## пользуются — с этапа 9 они спрашивают `NodeSignalController` парой
## (узел, подход); здесь он остался ради полноты осевой модели.
