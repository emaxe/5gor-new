...
## Размен зависит от размера города: отсев по габаритам — O(рёбер) на кадр,
## и при топологии раз в десять крупнее выгоднее станет хеш (свой, с ячейкой
## порядка радиуса миникарты, а не графовый с ячейкой 16 м).
