...
func _process(delta: float) -> void:
	...
	if mode == Mode.PED:
		# В пешем режиме курс камеры не доводится автоматически за персонажем.
		# Ввод перемещения (WASD) проецируется относительно камеры: автоповорот
		# камеры за персонажем создаёт петлю обратной связи (камера доворачивает ->
		# меняется вектор движения -> персонаж бегает по кругу).
		# Курсом камеры управляет игрок (мышь/стик/тач), а при смене режима
		# камера ставится ровно за спину через snap_to_target().
		pass
	elif _auto_return > 0.0:
		_auto_return -= delta
	else:
		_target_yaw = target_heading
